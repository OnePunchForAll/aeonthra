...
$event=[ordered]@{schema_version=1;created_at=(Get-Date).ToUniversalTime().ToString('o');hook='Stop';event_path=$EventPath;write_boundary='.codex-godmode/events/hooks.jsonl';mutates_shared_state=$false;source='aeonthra-hook-bus'}
