...
### [ENGINE/SHELL TRUTH-GATE] Deterministic semantic leak pass now suppresses wrapper surfaces all the way through shell and Atlas

**Files changed**: `packages/content-engine-v2/src/outputs/result.ts`, `packages/content-engine-v2/src/contracts/types.ts`, `packages/content-engine-v2/src/compatibility/benchmark-surface.ts`, `packages/content-engine-v2/src/benchmarks/run.ts`, `packages/content-engine-v2/src/benchmarks/score.ts`, `packages/content-engine-v2/src/tests/engine.test.ts`, `packages/content-engine-v2/src/tests/consumer-compat.test.ts`, `packages/content-engine-v2/src/tests/benchmark.test.ts`, `packages/content-engine-v2/src/fixtures/corpus.ts`, `apps/web/src/lib/workspace.ts`, `apps/web/src/lib/workspace.test.ts`, `apps/web/src/lib/shell-mapper.ts`, `apps/web/src/lib/shell-mapper.test.ts`, `apps/web/src/lib/atlas-skill-tree.ts`, `apps/web/src/lib/atlas-skill-tree.test.ts`, `apps/web/src/lib/atlas-shell.ts`, `apps/web/src/lib/atlas-shell.test.ts`
**What it does**: Replaces line-based requirement scraping with sentence-grounded extraction, removes plain pages from the default assignment-like lane, suppresses wrapper/noise page surfaces with explicit rejection codes, reattaches orphan concept pages to the explicit module they actually explain, blanks noisy shell assignment projections, and rewrites Atlas fallback labels/rewards into deterministic outcome-based unlocks. The benchmark surface now also scores shell-facing module, skill, checklist, and assignment projections instead of only raw engine output.
**Why**: The repo could still pass engine-level checks while live shell modules, chapter anchors, checklist lines, and Atlas skill labels leaked wrapper titles such as `Course Capture`, `Week 4 Discussion Forum`, and lazy `Explain ...` fallbacks.
**Downstream effects**: Mixed-noise pages no longer mint fake assignment mappings, thin discussion bundles now anchor on `Cognitive Load Theory`, suspicious dates stay unknown downstream, Atlas labels read as earned competencies, and the benchmark itself now fails if shell-facing regressions reappear.
**Skill refs**: `deterministic-study-truth-gate`, `shell-mapper-truth-gate`, `atlas-skill-tree-architect`, `regression-orchestrator`

### [ENGINE/HARNESS RE-PROOF] Page-backed concept pages no longer count as assignments, and the benchmark now scores that honestly

**Files changed**: `packages/content-engine-v2/src/outputs/result.ts`, `packages/content-engine-v2/src/benchmarks/score.ts`, `packages/content-engine-v2/src/fixtures/corpus.ts`, `packages/content-engine-v2/src/tests/engine.test.ts`, `packages/content-engine-v2/src/tests/consumer-compat.test.ts`, `packages/content-engine-v2/src/tests/benchmark.test.ts`
**What it does**: Removes the remaining page-backed assignment escape hatch from the engine, keeps focus themes grounded on concept labels instead of wrapper labels like `Course Capture`, flips page-only benchmark fixtures to expect zero assignments, and treats `expectedAssignmentTitles: []` as a real zero-surface requirement in benchmark scoring. The benchmark’s shell-facing integrity weight was also raised so live-shell failures matter enough to preserve the intended `delta >= 15` gate.
