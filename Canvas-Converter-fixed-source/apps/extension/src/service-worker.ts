...
type GenericPageCapturePayload = {
  title: string;
  url: string;
  text: string;
...
function sanitizeManualPageText(text: string): string {
  return text.replace(/\s+/g, " ").trim().slice(0, 240_000);
}

async function captureCurrentPageBundle(): Promise<{ ok: true; bundle: CaptureBundle; title: string } | { ok: false; message: string }> {
  const tab = await activeTab();
  if (!tab?.id || !tab.url) {
    return { ok: false, message: "Open a webpage first so AEONTHRA can capture it." };
  }

  if (!/^https?:/i.test(tab.url)) {
    return { ok: false, message: "AEONTHRA can only capture normal http(s) webpages as manual JSON exports." };
  }

  const payload = await scriptingExecuteFunction(
    { tabId: tab.id },
    () => {
      const root = (document.querySelector("main, article, [role='main']") as HTMLElement | null) ?? document.body;
      const clone = root.cloneNode(true) as HTMLElement;
      clone.querySelectorAll("script, style, noscript, svg, nav, aside, header, footer, form, button, input, select").forEach((node) => node.remove());
      const selectedText = window.getSelection?.()?.toString().trim() || "";
      const rawText = selectedText.length >= 120
        ? selectedText
        : ((clone.innerText || root.innerText || document.body.innerText || "").replace(/\s+/g, " ").trim());
      return {
        title: document.title || location.pathname || "Captured Page",
        url: location.href,
        text: rawText,
  ...
  const captured = payload as GenericPageCapturePayload | undefined;
  const text = sanitizeManualPageText(captured?.text ?? "");
  if (!captured || !text || text.length < 80) {
    return { ok: false, message: "AEONTHRA could not find enough readable text on the current page to export." };
  }

  const bundle = createManualCaptureBundle({
    title: captured.title || tab.title || "Captured Page",
    text,
    canonicalUrl: captured.url,
    kind: "page"
  });

  const host = (() => {
    try {
      return new URL(captured.url).host;
    } catch {
      return "unknown-host";
    }
  })();
    ...
    bundle: {
      ...
      items: bundle.items.map((item) => ({
        ...
        tags: [
          "manual-page-capture",
          `host:${host}`,
          ...(captured.selectedText ? ["selected-text-priority"] : [])
        ]
      }))
    }
  };
...
async function downloadCurrentPageJson(): Promise<{ ok: true; title: string } | { ok: false; message: string }> {
  const captured = await captureCurrentPageBundle();
  if (!captured.ok) {
    return captured;
  }

  const slug = captured.title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") || "page";
  const filename = `aeonthra-page-${slug}-${new Date().toISOString().replace(/[:.]/g, "-")}.json`;
  const url = `data:application/json;charset=utf-8,${encodeURIComponent(JSON.stringify(captured.bundle, null, 2))}`;
  await downloadsDownload({
    url,
    filename,
    saveAs: false
...
async function handleMessage(message: unknown, sender: chrome.runtime.MessageSender): Promise<unknown> {
  ...
  if (type === "aeon:download-current-page-json") {
    return downloadCurrentPageJson();
  }
