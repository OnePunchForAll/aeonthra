...
function Popup() {
  ...
  const handleDownloadCurrentPageJson = async () => {
    setError(null);
    setStatusMessage(null);
    try {
      const response = await sendMessageWithRetry<{ ok?: boolean; error?: string; message?: string }>({ type: "aeon:download-current-page-json" }, 2);
      if (response?.ok === false) {
        setError(response.error || response.message || "Current page export failed");
        return;
      }
      setStatusMessage(response?.message || "Downloaded current page JSON for manual review/import.");
    } catch (downloadError) {
      const message = downloadError instanceof Error ? downloadError.message : String(downloadError ?? "unknown error");
      setError(`Current page export failed: ${message}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      {statusMessage && !loading && !error && (
        ...
        >
          <p style={{ fontSize: "0.78rem", color: "#c9f7ff", margin: 0, lineHeight: 1.6 }}>{statusMessage}</p>
        </div>
      )}
      ...
      {!loading && ext?.state === "course-detected" && (
        <div>
          ...
          <button
            type="button"
            onClick={() => void handleDownloadCurrentPageJson()}
            style={{
              width: "100%",
              padding: "10px",
              background: "transparent",
      ...
      {!loading && ext?.state !== "course-detected" && (
        ...
        >
          ...
          <p style={{ fontSize: "0.85rem", color: "#b0b0d0", lineHeight: 1.6 }}>
            Navigate to a supported Canvas course surface for automatic capture, or export the current webpage as a manual JSON source for review/import.
          </p>
          <button
            type="button"
            onClick={() => void handleDownloadCurrentPageJson()}
