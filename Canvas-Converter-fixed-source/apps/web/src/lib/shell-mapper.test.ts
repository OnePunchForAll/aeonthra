...
vi.mock("./atlas-skill-tree", async () => {
  const actual = await vi.importActual<typeof import("./atlas-skill-tree")>("./atlas-skill-tree");
  return {
    ...actual,
    buildAtlasSkillTree: vi.fn((input) => actual.buildAtlasSkillTree(input))
  };
});
...
describe("shell mapper", () => {
  ...
  it("blanks noisy subtitle, tip, and reallyAsking text instead of projecting wrapper prose", () => {
    const bundle = createManualCaptureBundle({
      title: "Positive Reinforcement",
      text: [
        "Positive reinforcement increases the chance that a behavior will happen again by adding a rewarding consequence.",
        "You need to have JavaScript enabled in order to access this site. Course Capture Week 4 Discussion Forum reply to classmates."
      ].join(" "),
      canonicalUrl: "https://canvas.example.test/courses/1/assignments/12",
      kind: "assignment"
    });
    const learning = buildLearningBundle(bundle);
    const conceptId = learning.concepts[0]?.id ?? "positive-reinforcement";
    const sourceItemId = bundle.items[0]!.id;
    const mappedLearning = {
      ...learning,
      synthesis: {
        ...
        assignmentMappings: [{
          ...
          summary: "You need to have JavaScript enabled in order to access this site. Course Capture Week 4 Discussion Forum reply to classmates.",
          likelySkills: ["apply"],
          conceptIds: [conceptId],
          focusThemeIds: [],
          readinessEligible: false,
          readinessAcceptanceReasons: [],
          readinessRejectionReasons: ["Noisy wrapper text should not project as shell-facing assignment guidance."],
          likelyPitfalls: ["Do not repeat the wrapper title or browser fallback copy."],
          checklist: ["You need to have JavaScript enabled in order to access this site."],
          evidence: [makeEvidenceFragment(sourceItemId, "Positive reinforcement increases the chance that a behavior will happen again by adding a rewarding consequence.")]
        }]
      }
    };
    ...
    expect(assignment?.title).toBe("Positive Reinforcement");
    expect(assignment?.sub).toBe("");
    expect(assignment?.tip).toBe("");
    expect(assignment?.reallyAsking).toBe("");
    expect(assignment?.requirementLines.some((line) => /javascript enabled|course capture|week 4 discussion forum|reply to classmates/i.test(line))).toBe(false);
  });

  it("suppresses generic wrapper module names when a later chapter carries the real anchor", () => {
    const bundle = createGroundedUtilitarianBundle();
    const learning = buildLearningBundle(bundle);
    const conceptId = learning.concepts[0]?.id ?? "utilitarianism";
    const workspace = {
      tasks: [],
      chapters: [
        {
          id: "module-1",
          sourceItemId: "wrapper-item",
          title: "Course Capture",
          summary: "Course Capture orientation wrapper summary.",
          conceptIds: [conceptId],
          moduleKey: "module-1"
        },
        {
          id: "module-1-anchor",
          sourceItemId: "anchor-item",
          title: "Cognitive Load Theory",
          summary: "Cognitive Load Theory explains how working memory limits shape instruction.",
          conceptIds: [conceptId],
          moduleKey: "module-1"
        }
      ]
    ...
    expect(shellData.modules[0]?.title).toBe("Cognitive Load Theory");
    expect(shellData.modules[0]?.title).not.toBe("Course Capture");
  });

  it("withholds assignment skill implication when the basis is concept-only", () => {
    const bundle = createManualCaptureBundle({
      title: "Positive Reinforcement",
      text: "Positive reinforcement increases the chance that a behavior will happen again by adding a rewarding consequence.",
      canonicalUrl: "https://canvas.example.test/courses/1/assignments/13",
      kind: "assignment"
    });
    const learning = buildLearningBundle(bundle);
    const workspace = deriveWorkspace(bundle, learning, createEmptyProgress());
    ...
    expect(taskId).toBe(bundle.items[0]?.id);
    ...
    const conceptOnlyTree: AtlasSkillTree = {
      ...
      assignmentRequirements: [{
        assignmentId: taskId ?? "assignment-13",
        title: "Positive Reinforcement",
        summary: "Use the concept chain under assignment pressure.",
        skillIds: ["skill-foundation-positive-reinforcement"],
        basis: "concept-only",
        conceptIds: [learning.concepts[0]?.id ?? "positive-reinforcement"],
        focusThemeIds: [],
        checklist: [],
        pitfalls: [],
        evidence: []
    ...
    vi.mocked(buildAtlasSkillTree).mockReturnValueOnce(conceptOnlyTree);

    const shellData = mapToShellData(bundle, learning, workspace);

    expect(shellData.skillTree.assignmentRequirements[0]?.basis).toBe("concept-only");
    expect(shellData.assignments[0]?.skills).toEqual([]);
  });
