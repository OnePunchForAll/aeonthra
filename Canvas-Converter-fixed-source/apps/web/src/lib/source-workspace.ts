...
export function determineAppStage(input: {
  ...
  if (!input.canvasBundle && !input.textbookBundle) {
...
export function describeCanvasLoadState(bundle: CaptureBundle | null): SourceStatusCardState {
  if (!bundle) {
    return {
      headline: "Optional",
      detail: "No Canvas course bundle is loaded. You can still build a workspace from a textbook, pasted source, or manual webpage capture JSON."
    };
  }
...
export function describeTextbookLoadState(input: {
  ...
  const baseLead = input.canvasBundle
    ? "Canvas course data is already loaded."
    : "Support sources can be used even without a Canvas bundle.";

  if (input.uploadLabel) {
    return {
      state: "loading",
      headline: "Loading",
      detail: `${baseLead} ${input.uploadLabel}.`
    };
  }
  ...
  if (input.textbookBundle) {
    return {
      state: "ready",
      headline: "Ready",
      detail: `${input.textbookBundle.title} is loaded. You can synthesize now or add more support sources first.`
    };
  }
  ...
  if (input.importError) {
    return {
      ...
      detail: `${baseLead} ${input.importError}`
    };
  }
  ...
  return {
    state: "required",
    headline: "Optional",
    detail: `${baseLead} Add a PDF, DOCX, TXT, pasted notes, or manual page-capture JSON to strengthen the workspace before synthesis.`
  };
}
