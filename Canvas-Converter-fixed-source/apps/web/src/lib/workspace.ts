...
const REQUIREMENT_SENTENCE_PATTERN = /^(?:Write|Analyze|Discuss|Explain|Describe|Compare|Contrast|Identify|Evaluate|Assess|Cite|Include|Use|Submit|Complete|Answer|Respond to|Draft|Review|Read|Watch|Prepare)\b/i;
...
function hasGroundedWorkspaceSentence(item: CaptureItem): boolean {
  return stripCanvasMetadata(item.plainText)
    .split(/(?<=[.!?])\s+/)
    .map((entry) => entry.trim())
    .some((entry) =>
      entry.length >= 40
      && !hasChrome(entry)
      && !isWrapperTaskTitle(entry)
      && normalize(entry).split(/\s+/).filter((token) => token.length > 3).length >= 5
    );
}
...
function shouldSkipWorkspaceItem(item: CaptureItem): boolean {
  ...
  const hasGroundedSentence = hasGroundedWorkspaceSentence(item);
  if (chromeHits >= 2 && !hasGroundedSentence) {
    return true;
  }
  return hasChrome(item.title) && chromeHits >= 1 && !hasGroundedSentence;
}
...
function requirementLines(item: CaptureItem): string[] {
  ...
  for (const sentence of sourceText
    .split(/(?<=[.!?])\s+/)
    .map((entry) => entry.trim())
    .filter((entry) => entry.length >= 20 && entry.length <= 260)) {
    if (!REQUIREMENT_SENTENCE_PATTERN.test(sentence)) {
      continue;
    }
    if (hasChrome(sentence) || isNoisyChecklistLine(sentence)) {
      continue;
    }
...
function isNoisyChecklistLine(line: string): boolean {
  const cleaned = line.trim().replace(/\s+/g, " ");
  if (!cleaned || cleaned.length > 180) {
    return true;
  }
  if (hasChrome(cleaned)) {
    return true;
  }
  if (/^\|/.test(cleaned)) {
    return true;
  }
  if (TASK_TEXT_SKIP_PATTERNS.some((pattern) => pattern.test(cleaned))) {
    return true;
  }
  const meaningfulCount = normalize(cleaned).split(/\s+/).filter((token) => token.length > 3).length;
  return meaningfulCount < 3;
}

function sanitizeChecklistLines(lines: string[]): string[] | null {
  ...
  for (const entry of lines) {
    ...
    if (isNoisyChecklistLine(cleaned)) {
      return null;
    }
    ...
    seen.add(key);
    sanitized.push(cleaned.endsWith(".") ? cleaned : `${cleaned}.`);
  }

  return sanitized.length > 0 ? sanitized : null;
}

const WRAPPER_TASK_TITLE_PATTERNS = [
  /^course capture$/i,
  /^reply to classmates$/i,
  /^respond to classmates$/i,
  /^week\s+\d+\s+(?:discussion(?: forum| topic)?|forum|reply(?: to classmates)?|discussion forum)$/i,
  /^module\s+\d+\s+(?:discussion(?: forum| topic)?|forum|reply(?: to classmates)?|discussion forum)$/i,
  /^lesson\s+\d+\s+(?:discussion(?: forum| topic)?|forum|reply(?: to classmates)?|discussion forum)$/i,
  /^unit\s+\d+\s+(?:discussion(?: forum| topic)?|forum|reply(?: to classmates)?|discussion forum)$/i,
  /^orientation home$/i
] as const;

function isWrapperTaskTitle(title: string): boolean {
  const cleaned = cleanTaskTitle(title);
  const normalized = normalize(cleaned);
  return GENERIC_TASK_TITLE_PATTERNS.some((pattern) => pattern.test(normalized)) ||
    WRAPPER_TASK_TITLE_PATTERNS.some((pattern) => pattern.test(cleaned));
}

function isSpecificTaskTitle(title: string): boolean {
  return Boolean(title.trim()) && !isGenericTaskTitle(title) && !isWrapperTaskTitle(title) && !isFragmentaryTaskTitle(title);
}
...
function conceptAnchorForIds(
  conceptIds: string[],
  learning: LearningBundle
): { title: string; summary: string } | null {
  for (const conceptId of conceptIds) {
    const concept = learning.concepts.find((entry) => entry.id === conceptId);
    if (!concept) {
      continue;
    }
    return {
      title: concept.label,
      summary: readable(concept.summary || concept.definition || concept.label, 220)
    };
  }
  return null;
}

function syntheticModuleKey(moduleKey: string): boolean {
  return /^sequence-\d+$/i.test(moduleKey);
}

function chapterCandidateRank(task: {
  title: string;
  summary: string;
  conceptIds: string[];
  kind: TaskKind;
  ...
  const summary = stripCanvasMetadata(task.summary);
  const summaryTokens = meaningful(summary);
  let score = task.conceptIds.length * 4;

  if (task.kind === "page") {
    score += 8;
  }

  if (isSpecificTaskTitle(title)) {
    score += 6;
  } else if (isWrapperTaskTitle(title)) {
    score -= 10;
  } else if (isGenericTaskTitle(title)) {
    score -= 4;
  }
  ...
  if (summaryTokens.length >= 8) {
    score += 2;
  }
  score += Math.min(summaryTokens.length, 10) * 0.5;
...
function chooseBestChapterCandidate(candidates: Array<{
  title: string;
  summary: string;
  conceptIds: string[];
  kind: TaskKind;
  ...
  return [...candidates].sort((left, right) => chapterCandidateRank(right) - chapterCandidateRank(left))[0]!;
}
...
export function deriveWorkspace(bundle: CaptureBundle, learning: LearningBundle, progress: AppProgress): {
  ...
  const entries = bundle.items
    .map((item, index) => {
      ...
      const directConceptIds = assignmentIntel?.conceptIds ?? [];
      const overlapConceptIds = directConceptIds.length > 0 ? [] : relatedConceptIds(item, learning);
      const conceptIds = Array.from(new Set([
        ...directConceptIds,
        ...overlapConceptIds
      ]));
      const plannedDate = dueDate ?? capturedAt + index * DAY;
      const projectedTitle = assignmentIntel?.title?.trim() ?? "";
      const projectedSummary = assignmentIntel?.summary?.trim() ?? "";
      const derivedTitle = deriveTaskTitle(item);
      const title = isSpecificTaskTitle(projectedTitle) ? projectedTitle : derivedTitle;
      const summary = projectedSummary && !isNoisyChecklistLine(projectedSummary)
        ? projectedSummary
        : cleanSummary(item);
      const sanitizedChecklist = assignmentIntel?.checklist?.length
        ? sanitizeChecklistLines(assignmentIntel.checklist)
        : null;
      const requirements = sanitizedChecklist ?? requirementLines(item);
      return {
        id: item.id,
  ...
  entries.forEach((entry, index) => {
    if (!syntheticModuleKey(entry.moduleKey) || entry.conceptIds.length === 0) {
      return;
    }

    const bestExplicitModule = entries
      .map((candidate, candidateIndex) => {
        if (candidateIndex === index || syntheticModuleKey(candidate.moduleKey)) {
          return null;
        }
        const overlap = candidate.conceptIds.filter((conceptId) => entry.conceptIds.includes(conceptId)).length;
        if (overlap === 0) {
          return null;
        }
        return {
    ...
    if (bestExplicitModule) {
      entry.moduleKey = bestExplicitModule.moduleKey;
    }
  });
  ...
  const tasks = entries
    .filter((task) => {
      ...
      if (isWrapperTaskTitle(task.title) && !isSpecificTaskTitle(assignmentIntelBySourceItemId.get(item.id)?.title ?? "")) return false;
  ...
  const chapterCandidatesByModule = new Map<string, typeof allChapterCandidates>();
  for (const candidate of allChapterCandidates) {
    const current = chapterCandidatesByModule.get(candidate.moduleKey) ?? [];
    current.push(candidate);
    chapterCandidatesByModule.set(candidate.moduleKey, current);
  }

  const moduleOrder = new Map<string, number>();
  allChapterCandidates.forEach((candidate, index) => {
    if (!moduleOrder.has(candidate.moduleKey)) {
      moduleOrder.set(candidate.moduleKey, index);
    }
  });

  const chapters = [...chapterCandidatesByModule.entries()]
    .map(([, candidates]) => chooseBestChapterCandidate(candidates))
    .sort((left, right) => (moduleOrder.get(left.moduleKey) ?? 0) - (moduleOrder.get(right.moduleKey) ?? 0))
    .map((task) => {
      const conceptAnchor = conceptAnchorForIds(task.conceptIds, learning);
      const title = (isGenericTaskTitle(task.title) || isWrapperTaskTitle(task.title)) && conceptAnchor?.title
        ? conceptAnchor.title
        : task.title;
      const summary = ((isGenericTaskTitle(task.title) || isWrapperTaskTitle(task.title)) && conceptAnchor?.summary)
        ? conceptAnchor.summary
        : task.summary;
      return {
        id: task.moduleKey,
