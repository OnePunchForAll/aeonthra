...
const GENERIC_SKILL_TITLE_PATTERNS = [
  /^course capture$/i,
  /^reply to classmates?$/i,
  /^respond to classmates?$/i,
  /^(?:week|module|chapter|lesson|unit)\s+\d+\b/i,
...
function isGenericSkillTitle(value: string | null | undefined): boolean {
  const normalized = normalizeSkillTitle(value ?? "");
  return !normalized
    || GENERIC_SKILL_TITLE_PATTERNS.some((pattern) => pattern.test(normalized));
}

function conceptChainLabel(
  conceptIds: string[],
  conceptById: Map<string, ConceptLike>,
  limit = 2
): string {
  const names = unique(
    conceptIds
      .map((conceptId) => conceptById.get(conceptId)?.name)
      .filter((name): name is string => Boolean(name))
  ).slice(0, limit);
  if (names.length === 0) {
    return "the source-backed skill chain";
  }
  if (names.length === 1) {
    return names[0]!;
  }
...
function moduleOutcomeLabel(module: ModuleLike, conceptById: Map<string, ConceptLike>): string {
  if (!isGenericSkillTitle(module.title)) {
    return module.title;
  }
  return conceptChainLabel(module.concepts, conceptById);
}
...
function bestVerb(
  ...
  const requested = verbs[0]?.trim().toLowerCase();
  if (requested) {
    if (requested === "explain") {
      return conceptCount > 1 ? "connect" : "grasp";
    }
    if (requested === "synthesize") {
      return conceptCount > 1 ? "connect" : "apply";
    }
    return requested;
  }
  return conceptCount > 1 ? "connect" : fallback;
}

function trustedAssignmentLaneLabel(
  rawTitle: string | null | undefined,
  conceptIds: string[],
  conceptById: Map<string, ConceptLike>
): string {
  const title = rawTitle?.trim() ?? "";
  if (!isGenericSkillTitle(title)) {
    return shortTitle(title);
  }
  return conceptChainLabel(conceptIds, conceptById);
}
...
export function buildAtlasSkillTree(input: BuildAtlasSkillTreeInput): AtlasSkillTree {
  ...
  const foundationNodes: AtlasSkillNode[] = foundationConceptIds.flatMap((conceptId) => {
    ...
    const supportingVerb = bestVerb(supportingIntel?.likelySkills ?? [], 1, "grasp");
    ...
    return [{
      ...
      rewardLabel: moduleId
        ? `Earned by finishing ${moduleOutcomeLabel(modules.find((module) => module.id === moduleId) ?? {
          id: moduleId,
          title: "",
          desc: "",
          concepts: []
  ...
  const themeEntries: Array<{ themeId: string; node: AtlasSkillNode }> = [];
  const themeNodeBySignature = new Map<string, AtlasSkillNode>();
  for (const theme of synthesis.focusThemes) {
    ...
    const signature = `${focusVerb}:${conceptIds.slice().sort().join("|")}`;
    let node = themeNodeBySignature.get(signature);

    if (!node) {
      node = {
        id: `skill-applied-${slug(signature)}`,
        label: `${capitalize(focusVerb)} ${conceptChainLabel(conceptIds, conceptById)}`,
        kind: "applied",
        ...
        chapterIds,
        rewardLabel: moduleId
          ? `Chapter reward from ${moduleOutcomeLabel(modules.find((module) => module.id === moduleId) ?? {
            id: moduleId,
            title: "",
            desc: "",
            concepts: []
          ...
          : null,
        branchId: moduleId ?? signature,
        tier: 2
      };
      themeNodeBySignature.set(signature, node);
    } else if (theme.summary.length > node.summary.length) {
      node.summary = theme.summary;
    }

    themeEntries.push({ themeId: theme.id, node });
  }

  const themeNodes = [...themeNodeBySignature.values()];
  ...
  const transferNodes: AtlasSkillNode[] = assignments.flatMap((assignment) => {
    ...
    return [{
      ...
      label: `Transfer ${trustedAssignmentLaneLabel(intel?.title ?? assignment.title, conceptIds, conceptById)}`,
      ...
      chapterIds,
      rewardLabel: moduleId
        ? `Feeds the ${moduleOutcomeLabel(modules.find((module) => module.id === moduleId) ?? {
          id: moduleId,
          title: "",
          desc: "",
          concepts: []
        }, conceptById)} assignment lane`
        : null,
  ...
  const readinessNodes: AtlasSkillNode[] = assignments.flatMap((assignment) => {
    ...
    return [{
      ...
      label: `Prepare ${trustedAssignmentLaneLabel(intel?.title ?? assignment.title, conceptIds, conceptById)}`,
  ...
  const chapterRewardNodes: AtlasSkillNode[] = modules.flatMap((module) => {
    ...
    return [{
      ...
      label: `Unlock ${moduleOutcomeLabel(module, conceptById)}`,
      kind: "chapter-reward" as const,
      summary: `Complete ${moduleOutcomeLabel(module, conceptById)} cleanly enough to cash out the chapter skill reward.`,
      ...
      rewardLabel: `Chapter reward for ${moduleOutcomeLabel(module, conceptById)}`,
      branchId: module.id,
      tier: 5
    }];
  });
  ...
  const masteryNodes: AtlasSkillNode[] = modules.flatMap((module) => {
    ...
    return [{
      ...
      label: `Master ${moduleOutcomeLabel(module, conceptById)}`,
      ...
      focusVerb: "master",
      chapterIds: [module.id],
      rewardLabel: `Culmination reward for ${moduleOutcomeLabel(module, conceptById)}`,
      branchId: module.id,
      tier: 6
    }];
  });
  ...
  const moduleGroups = modules
    .map((module) => ({
      id: `group-${module.id}`,
      title: moduleOutcomeLabel(module, conceptById),
      summary: module.desc,
      moduleId: module.id,
  ...
  const chapterRewards = modules
    .map((module) => ({
      id: `reward-${module.id}`,
      moduleId: module.id,
      title: `${moduleOutcomeLabel(module, conceptById)} rewards`,
      summary: `Finish the chapter cluster and earn the skills that carry ${moduleOutcomeLabel(module, conceptById)} into assignments.`,
      skillIds: nodes
