...
describe("atlas skill tree", () => {
  ...
  it("uses concrete skill-outcome labels instead of fallback explain or synthesize wording", () => {
    const tree = buildAtlasSkillTree({
      concepts: [
        {
          id: "virtue",
          name: "Virtue Ethics",
      ...
      modules: [
        {
          id: "module-1",
          title: "Course Capture",
          desc: "A generic wrapper chapter title that should not leak into Atlas labels.",
          concepts: ["virtue", "deontology"]
        }
      ],
      distinctions: [
    ...
    expect(labels.some((label) => /^(Explain|Synthesize)\s/i.test(label))).toBe(false);
    expect(labels.some((label) => label.includes("Course Capture"))).toBe(false);
  });

  it("deduplicates near-identical applied branches instead of emitting clone nodes", () => {
    const tree = buildAtlasSkillTree({
      concepts: [
        {
          id: "virtue",
      ...
      modules: [
        {
          ...
          desc: "Build the baseline concept before any application.",
          concepts: ["virtue"]
        }
      ],
      distinctions: [],
        ...
        focusThemes: [
          {
            id: "theme-a",
            label: "moral reasoning under pressure",
            summary: "Apply the chapter idea in a case.",
            verbs: ["apply"],
            conceptIds: ["virtue"]
          },
          {
    ...
    const appliedNodes = tree.nodes.filter((node) => node.kind === "applied");

    expect(appliedNodes).toHaveLength(1);
    expect(new Set(appliedNodes.map((node) => node.branchId)).size).toBe(1);
  });
});
