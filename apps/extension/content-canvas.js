...
var BRIDGE_SOURCE = "learning-freedom-bridge";
var captureItemKinds = [
  "selection",
  "page",
  "article",
  "assignment",
...
var CaptureResourceSchema = external_exports.object({
  id: external_exports.string(),
  title: external_exports.string(),
  url: external_exports.string().url(),
  kind: external_exports.enum(resourceKinds),
  sourceItemId: external_exports.string()
});
var CaptureItemSchema = external_exports.object({
  id: external_exports.string(),
  ...
  headingTrail: external_exports.array(external_exports.string()).default([]),
...
var CaptureBundleSchema = external_exports.object({
  schemaVersion: external_exports.string(),
  source: external_exports.enum(["extension-capture", "manual-import", "demo"]),
  title: external_exports.string(),
  capturedAt: external_exports.string(),
  items: external_exports.array(CaptureItemSchema),
  resources: external_exports.array(CaptureResourceSchema),
  manifest: external_exports.object({
    itemCount: external_exports.number().int().nonnegative(),
    resourceCount: external_exports.number().int().nonnegative(),
    captureKinds: external_exports.array(external_exports.enum(captureItemKinds)),
    sourceUrls: external_exports.array(external_exports.string().url())
  }),
  captureMeta: external_exports.object({
    mode: external_exports.enum(["complete", "learning"]).optional(),
    courseId: external_exports.string().optional(),
    courseName: external_exports.string().optional(),
    sourceHost: external_exports.string().optional(),
    stats: external_exports.object({
      totalItemsVisited: external_exports.number().int().nonnegative(),
      totalItemsCaptured: external_exports.number().int().nonnegative(),
      totalItemsSkipped: external_exports.number().int().nonnegative(),
      totalItemsFailed: external_exports.number().int().nonnegative(),
    ...
    rawHtmlArchive: external_exports.record(external_exports.string()).optional()
  }).optional()
});
var LearningConceptSchema = external_exports.object({
  id: external_exports.string(),
  label: external_exports.string(),
  score: external_exports.number(),
  summary: external_exports.string(),
  primer: external_exports.string(),
  mnemonic: external_exports.string(),
  excerpt: external_exports.string(),
  definition: external_exports.string(),
  stakes: external_exports.string(),
  commonConfusion: external_exports.string(),
  transferHook: external_exports.string(),
  category: external_exports.string().optional(),
  keywords: external_exports.array(external_exports.string()).default([]),
  sourceItemIds: external_exports.array(external_exports.string()),
  relatedConceptIds: external_exports.array(external_exports.string())
});
var ConceptRelationSchema = external_exports.object({
  fromId: external_exports.string(),
  toId: external_exports.string(),
  type: external_exports.enum(relationTypes),
  label: external_exports.string(),
  strength: external_exports.number()
});
var EngineProfileSchema = external_exports.object({
  id: external_exports.enum(engineIds),
  title: external_exports.string(),
  thesis: external_exports.string(),
  signature: external_exports.string(),
  contribution: external_exports.string(),
  moves: external_exports.array(external_exports.string())
});
var EvidenceFragmentSchema = external_exports.object({
  label: external_exports.string(),
  excerpt: external_exports.string(),
  sourceItemId: external_exports.string()
});
var PhaseSubmodeSchema = external_exports.object({
  id: external_exports.string(),
  title: external_exports.string(),
  engineIds: external_exports.array(external_exports.enum(engineIds)).min(1),
  durationMinutes: external_exports.number().int().positive(),
  challengeLevel: external_exports.enum(challengeLevels),
  objective: external_exports.string(),
  setup: external_exports.string(),
  prompt: external_exports.string(),
  tasks: external_exports.array(external_exports.string()).min(2),
  reflection: external_exports.string(),
  conceptIds: external_exports.array(external_exports.string()).min(1),
  evidence: external_exports.array(EvidenceFragmentSchema).min(1)
});
var MegaPhaseSchema = external_exports.object({
  id: external_exports.enum(phaseIds),
  title: external_exports.string(),
  tagline: external_exports.string(),
  summary: external_exports.string(),
  totalMinutes: external_exports.number().int().positive(),
  winCondition: external_exports.string(),
  conceptIds: external_exports.array(external_exports.string()).min(1),
  submodes: external_exports.array(PhaseSubmodeSchema).length(4)
});
var LearningProtocolSchema = external_exports.object({
  totalMinutes: external_exports.number().int().positive(),
  phases: external_exports.array(MegaPhaseSchema).length(5)
});
var NeuralForgeCardSchema = external_exports.object({
  id: external_exports.string(),
  title: external_exports.string(),
  focus: external_exports.string(),
  summary: external_exports.string(),
  prompt: external_exports.string(),
  supportLine: external_exports.string(),
  actions: external_exports.array(external_exports.string()).min(1),
  conceptIds: external_exports.array(external_exports.string()).min(1),
  evidence: external_exports.array(EvidenceFragmentSchema).min(1)
});
var NeuralForgePhaseSchema = external_exports.object({
  id: external_exports.enum(neuralForgePhaseIds),
  title: external_exports.string(),
  tagline: external_exports.string(),
  purpose: external_exports.string(),
  durationMinutes: external_exports.number().int().positive(),
  ambientLines: external_exports.array(external_exports.string()).min(1),
  cards: external_exports.array(NeuralForgeCardSchema).min(1)
});
var NeuralForgeSchema = external_exports.object({
  totalMinutes: external_exports.number().int().positive(),
  atmosphere: external_exports.string(),
...
var LearningBundleSchema = external_exports.object({
  ...
  sourceBundleTitle: external_exports.string(),
  concepts: external_exports.array(LearningConceptSchema),
...
var BridgeEnvelopeSchema = external_exports.object({
  source: external_exports.literal(BRIDGE_SOURCE)
});
var NFPackReadyMessageSchema = BridgeEnvelopeSchema.extend({
  type: external_exports.literal("NF_PACK_READY"),
  pack: CourseKnowledgePackSchema
});
var NFPackAckMessageSchema = BridgeEnvelopeSchema.extend({
  type: external_exports.literal("NF_PACK_ACK"),
  packId: external_exports.string()
});
var NFImportRequestMessageSchema = BridgeEnvelopeSchema.extend({
  type: external_exports.literal("NF_IMPORT_REQUEST")
});
var NFImportResultMessageSchema = BridgeEnvelopeSchema.extend({
  type: external_exports.literal("NF_IMPORT_RESULT"),
  success: external_exports.boolean(),
  packId: external_exports.string().optional(),
  error: external_exports.string().optional()
});
var NFPingMessageSchema = BridgeEnvelopeSchema.extend({
  type: external_exports.literal("NF_PING")
});
var NFPongMessageSchema = BridgeEnvelopeSchema.extend({
  type: external_exports.literal("NF_PONG")
});
var BridgeMessageSchema = external_exports.discriminatedUnion("type", [
...
var LEARNING_MIN_SCORE = 10;
var state = window.__aeonthraCaptureState ??= {
  running: false,
  paused: false,
  cancelled: false
};
...
function parseLinkHeader(linkHeader) {
  ...
  const nextPart = linkHeader.split(",").map((part) => part.trim()).find((part) => /rel=\"?next\"?/.test(part));
  const match = nextPart?.match(/<([^>]+)>/);
  return match?.[1] ?? null;
}
async function fetchAllJson(url, settings) {
  const results = [];
  let next = url;
  while (next) {
    let delay = settings.retryBackoffMs;
...
function cleanHtmlFragment(html) {
  return html.replace(/\sdata-[^=]+=\"[^\"]*\"/g, "").replace(/\sclass=\"[^\"]*\"/g, "").replace(/\sid=\"[^\"]*\"/g, "").trim();
}
...
function learningBlocks(doc) {
  for (const selector of CANVAS_CHROME_SELECTORS) {
    doc.querySelectorAll(selector).forEach((node) => node.remove());
  }
  const candidates = Array.from(doc.querySelectorAll("main h1, main h2, main h3, main p, main li, article h1, article h2, article h3, article p, article li, .user_content p, .user_content li, .show-content p, .show-content li")).map((node) => normalizeWhitespace(node.textContent ?? "")).filter(Boolean);
  const deduped = [];
...
function bestContentRoot(doc) {
  return doc.querySelector("main .user_content.enhanced") || doc.querySelector("main .show-content") || doc.querySelector(".user_content.enhanced") || doc.querySelector("main") || doc.body;
}
function titleTrail(title, queueItem) {
  return [title, queueItem.moduleName ?? "", queueItem.type].map((entry) => entry.trim()).filter(Boolean);
}
function buildCaptureItem(input) {
  ...
  return {
    ...
    tags: Array.from(/* @__PURE__ */ new Set([input.queueItem.type, ...input.tags ?? []])),
    capturedAt,
    contentHash: stableHash(normalizeWhitespace(input.plainText))
  };
}
function dateLine(label, rawValue) {
...
function extractRubricText(doc) {
  const rubricRoot = doc.querySelector(".rubric, .rubric_table, [data-testid='rubric-assessment-table']");
  if (!rubricRoot) {
    return [];
  }
  return Array.from(rubricRoot.querySelectorAll("tr")).map((row) => normalizeWhitespace(row.textContent ?? "")).filter((line) => line.length >= 18).slice(0, 24);
}
function composeLearningText(lines) {
  return lines.filter((line) => Boolean(line && normalizeWhitespace(line))).join("\n");
}
function buildAssignmentPayload(queueItem, html, mode) {
  const doc = parseHtml(html);
  const title = normalizeWhitespace(
    doc.querySelector("h1.title, .assignment-title .title-content, h1")?.textContent || queueItem.title
  );
  ...
  const lines = composeLearningText([
    ...
    Array.isArray(metadata.submission_types) && metadata.submission_types.length > 0 ? `Submission types ${metadata.submission_types.join(", ")}` : null,
    ...learning,
    ...rubric
  ]);
  ...
  const item = buildCaptureItem({
    ...
    title,
    plainText: lines,
    html: mode === "complete" ? cleanHtmlFragment(contentRoot.innerHTML) : void 0,
    tags: ["canvas", "assignment"]
  });
  return {
    item,
    resources: collectLinks(contentRoot, item.id),
    rawHtml: mode === "complete" ? html : void 0
  };
}
function buildDiscussionPayload(queueItem, html, mode) {
  const doc = parseHtml(html);
  ...
  const item = buildCaptureItem({
    ...
    title,
    plainText: prompt,
    html: mode === "complete" ? cleanHtmlFragment(promptRoot.innerHTML) : void 0,
...
function buildPagePayload(queueItem, html, mode) {
  const doc = parseHtml(html);
  ...
  const item = buildCaptureItem({
    ...
    title,
    plainText: text,
    html: mode === "complete" ? cleanHtmlFragment(root.innerHTML) : void 0,
    tags: ["canvas", "page"]
  });
  return {
    item,
...
function buildQuizPayload(queueItem, html, mode) {
  const doc = parseHtml(html);
...
function buildApiOnlyPayload(queueItem, mode) {
  const metadata = queueItem.raw ?? {};
  let lines = [queueItem.title];
  if (queueItem.type === "module") {
    const rawItems = Array.isArray(metadata.items) ? metadata.items : [];
    const itemLines = rawItems.map((entry) => normalizeWhitespace(`${entry.title ?? entry.page_url ?? entry.type ?? "Module item"}`)).filter(Boolean).slice(0, 40);
    lines = [
      queueItem.title,
  ...
  const item = buildCaptureItem({
    ...
    plainText: plain,
    html: mode === "complete" && typeof metadata["html"] === "string" ? cleanHtmlFragment(String(metadata["html"])) : void 0,
    tags: ["canvas", queueItem.type]
  });
  const resources = typeof metadata["url"] === "string" ? [{
...
async function discoverCourse(payload) {
...
async function emitWarning(jobId, warning) {
...
async function captureQueueItem(job, queueItem) {
