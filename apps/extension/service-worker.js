...
function captureBundleId(bundle) {
  return stableHash(
    JSON.stringify({
      title: bundle.title,
...
function createEmptyBundle(title = "Untitled capture") {
  return {
...
// src/core/storage.ts
var SETTINGS_KEY = "aeonthra:settings";
var SETTINGS_FALLBACK_KEY = "aeonthra:settings:local";
...
async function resetRuntimeState(course = null) {
...
async function writeHistorySummaries(history) {
...
async function readCaptureRecord(id) {
  ...
  return stored[captureRecordKey(id)] ?? null;
}
async function writeCaptureRecord(record) {
...
var PARTIAL_BUNDLE_KEY = "aeonthra:partial-bundle";
var PARTIAL_WARNINGS_KEY = "aeonthra:partial-warnings";
var PARTIAL_RAW_HTML_KEY = "aeonthra:partial-raw-html";
...
async function getPartialBundle() {
...
async function ensureTabLoaded(tabId, timeoutMs = 15e3) {
...
async function sendCanvasMessage(tabId, payload) {
...
async function broadcastOverlay(state) {
  const targets = [state.sourceTabId, state.captureTabId].filter((value) => typeof value === "number");
  await Promise.all(
...
async function requestWorkspaceImport(tabId) {
  await ensureTabLoaded(tabId, 8e3);
  try {
    const response = await tabsSendMessage(tabId, { type: "bridge-request-import" });
...
async function finalizeCapture(jobId, mode, course, stats, cancelled = false) {
  ...
  const finalized = {
    ...
    captureMeta: {
      ...
      warnings,
      rawHtmlArchive: mode === "complete" ? rawHtmlArchive : void 0
    }
  };
...
async function startCapture(mode) {
  ...
  try {
    const response = await sendCanvasMessage(captureTab.id, {
...
async function setCaptureControl(action) {
  const runtime = await readRuntimeState();
  if (!runtime.captureTabId || !runtime.jobId) {
    return { ok: false, message: "No active capture is available." };
  }
  try {
    const response = await sendCanvasMessage(runtime.captureTabId, {
  ...
  if (action === "pause") {
    ...
    await updateRuntime({ status: "cancelled", phaseLabel: "Capture Cancelled", finishedAt: (/* @__PURE__ */ new Date()).toISOString() });
  }
  return { ok: true };
}
async function downloadCapture(captureId) {
...
async function openClassroom(captureId) {
  const targetId = captureId ?? await latestCaptureId();
  if (!targetId) {
...
async function handleMessage(message, sender) {
  ...
  if (type === "START_CAPTURE" || type === "aeon:start-capture") {
    const response = await startCapture(message.mode ?? "learning");
    return {
      ...response,
  ...
  if (type === "aeon:delete-capture") {
    await deleteCaptureRecord(message.captureId);
    return { ok: true };
  }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  (async () => {
    try {
      ...
      sendResponse({ ok: true, ...result && typeof result === "object" ? result : { result } });
    } catch (err) {
