...
export default function App(){
  ...
  const card={background:CARD,border:`1px solid ${BDR}`,borderRadius:20,padding:"36px 40px"};
  const ey={fontSize:".76rem",fontWeight:800,letterSpacing:".16em",textTransform:"uppercase",color:MUTE,marginBottom:16};
  const hd=(s)=>({fontSize:s+"rem",fontWeight:700,margin:0,lineHeight:1.3,color:TXT});
  const btn=(bg,fg)=>({background:bg,color:fg,border:"none",padding:"14px 32px",borderRadius:14,fontWeight:700,fontSize:".88rem",cursor:"pointer",letterSpacing:".04em",transition:"all 220ms cubic-bezier(.22,1,.36,1)"});
  ...
  return(
  <div style={{minHeight:"100vh",background:BG,backgroundImage:"radial-gradient(ellipse at 50% 0%,#0a0a20 0%,#020208 55%)",color:TXT,fontFamily:"'Segoe UI',system-ui,sans-serif",fontSize:"17px"}}>
    <nav style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"18px 36px",borderBottom:`1px solid rgba(0,240,255,.06)`,position:"sticky",top:0,zIndex:100,background:"rgba(2,2,8,.94)",backdropFilter:"blur(16px)"}}>
      <div style={{display:"flex",alignItems:"center",gap:16}}>
        <span style={{fontWeight:900,fontSize:"1.3rem",letterSpacing:".14em",color:CYAN,textShadow:"0 0 28px rgba(0,240,255,.3)"}}>AEONTHRA</span>
      ...
      <div style={{display:"flex",gap:4}}>
        {[["home","Home"],["timeline","Timeline"],["explore","Concepts"],["forge","Learn"],["compare","Compare"],["oracle","Oracle"]].map(([id,lb])=>(
    ...
    <main style={{maxWidth:1200,margin:"0 auto",padding:"36px 32px 100px",opacity:fade?1:0,transition:"opacity 180ms ease"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:24}}>
          ...
          <div style={{display:"flex",gap:40}}>
            {[[mastered+"/"+cc.length,"Mastered"],[pct(avg),"Overall"],[ASSIGNMENTS.length,"Tasks"]].map(([val,lb],i)=>(
              <div key={i} style={{textAlign:"center"}}><div style={{fontSize:"2rem",fontWeight:700,fontVariantNumeric:"tabular-nums"}}>{val}</div><div style={{fontSize:".72rem",color:MUTE,letterSpacing:".12em",textTransform:"uppercase",marginTop:4}}>{lb}</div></div>
            ))}
          </div>
        </div>
      </div>
    ...
    {/* ═══ TIMELINE ═══ */}
    {v==="timeline"&&<div>
      <h2 style={hd(1.4)}>Semester Timeline</h2>
      <p style={{color:T2,fontSize:".92rem",margin:"8px 0 28px"}}>Your assignments laid out by deadline — see what's coming and how ready you are.</p>
      <div style={{display:"flex",gap:16,overflowX:"auto",paddingBottom:16}}>
        {[...ASSIGNMENTS].sort((a,b)=>a.due-b.due).reduce((weeks,a)=>{
          const wk=a.due<=7?"This Week":a.due<=14?"Next Week":a.due<=28?"This Month":"Later";
          const existing=weeks.find(w=>w.label===wk);
          if(existing){existing.items.push(a);}else{weeks.push({label:wk,items:[a]});}
          return weeks;
        },[]).map(week=>(
          <div key={week.label} style={{flex:"0 0 320px",minWidth:320}}>
            <div style={{fontSize:".72rem",fontWeight:800,letterSpacing:".16em",textTransform:"uppercase",color:CYAN,marginBottom:14,paddingBottom:10,borderBottom:`1px solid ${BDR}`}}>{week.label}</div>
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              {week.items.map(a=>{
                const rdy=a.concepts.every(id=>(cc.find(c=>c.id===id)?.mastery??0)>=.6);
                const urgency=a.due<=3?RED:a.due<=7?"#ff8800":CYAN;
                return(
                  <button key={a.id} onClick={()=>go("assignment",{a})} style={{...card,padding:"20px 24px",cursor:"pointer",borderLeft:`4px solid ${urgency}`,textAlign:"left",width:"100%",transition:"all 250ms"}}>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                      <span style={{fontSize:"1.1rem"}}>{ti(a.type)}</span>
                      <span style={{fontSize:".68rem",fontWeight:700,padding:"3px 10px",borderRadius:14,...(rdy?{color:TEAL,background:`${TEAL}0f`}:{color:"#ff8800",background:"#ff88000f"})}}>{rdy?"Ready":"Prepare"}</span>
                    </div>
                    <div style={{fontSize:"1rem",fontWeight:700,marginBottom:4}}>{a.title}</div>
                    <div style={{fontSize:".82rem",color:T2,marginBottom:8}}>{a.sub}</div>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <span style={{fontSize:".76rem",color:MUTE}}>{a.pts} pts</span>
                      <span style={{fontSize:".76rem",color:urgency,fontWeight:600}}>Due in {a.due} days</span>
                    </div>
                    <div style={{display:"flex",gap:4,marginTop:10,flexWrap:"wrap"}}>
                      {a.concepts.map(id=>{const c=cc.find(x=>x.id===id);return c?<span key={id} style={{fontSize:".65rem",padding:"2px 8px",borderRadius:10,background:`${mc(c.mastery)}12`,color:mc(c.mastery),border:`1px solid ${mc(c.mastery)}22`}}>{c.name}</span>:null;})}
                    </div>
                  </button>
                );
              })}
    ...
    <style>{`@keyframes fadeUp{from{opacity:0;transform:translateY(14px)}to{opacity:1;transform:translateY(0)}}*:focus-visible{outline:2px solid #00f0ff;outline-offset:3px;border-radius:10px}button:hover{filter:brightness(1.12)}button:active{transform:scale(.97);transition:transform 80ms ease}::selection{background:rgba(0,240,255,.2);color:#fff}::-webkit-scrollbar{width:6px;height:6px}::-webkit-scrollbar-track{background:transparent}::-webkit-scrollbar-thumb{background:rgba(0,240,255,.15);border-radius:3px}::-webkit-scrollbar-thumb:hover{background:rgba(0,240,255,.3)}`}</style>
  </div>);
}
